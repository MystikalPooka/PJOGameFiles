namespace Script {
    ...
    public partial class Main {
        ...
        public static bool CheckStatusProtection(ICharacter character, ICharacter attacker, IMap map, string status, string tag, bool msg, PacketHitList hitlist) {
            ...
            switch (statusParam[0]) {
                ...
                case "Embargo": {
                    ...
                    break;
                case "Transform": {//The status
                
                if (character.VolatileStatus.GetStatus("Transform") != null) {
                if (msg) {
                hitlist.AddPacketToMap(map, PacketBuilder.CreateBattleMsg(character.Name + "transformed into into the target!", Text.Magenta), character.X, character.Y, 10);
                }
            ...
            if (character.VolatileStatus.GetStatus(name) == null) {
                ...
                switch (nameSegment[0]) {
                    ...
                    case "SkyDrop": {
                        ...
                        break;
                    case "Transform": {
                    if (msg) hitlist.AddPacketToMap(map, PacketBuilder.CreateBattleMsg(character.Name + " transformed into the target!", Text.Magenta), character.X, character.Y, 10);
                     }
                    break;    
                    case "SemiInvul": {
                        RefreshCharacterSprite(character, map, hitlist);    
            ...
            if (statusToRemove != null) {
                ...
                switch (nameSegment[0]) {
                    ...
                    case "SkyDrop": {
                        ...
                        break;
                    case "SemiInvul": {
                    	hitlist.AddPacketToMap(map, PacketBuilder.CreateBattleMsg(character.Name + " SEMI INVUL OVER HERE", Text.Magenta), character.X, character.Y, 10);
        ...
        public static void ChangeToDefender(BattleSetup setup, ICharacter character, IMap map, PacketHitList hitlist) {
        PacketHitList.MethodStart(ref hitlist);
        ExtraStatus status;
        setup.Attacker.Sprite = setup.Defender.Sprite;
        ...
        public static void RefreshCharacterSprite(ICharacter character, IMap map, PacketHitList hitlist) {
            ...
            if (character.VolatileStatus.GetStatus("Substitute") != null) {
                 ...
                 character.Sprite = 111;
            }

			if (character.VolatileStatus.GetStatus("Transform") != null) {
                
                character.Sprite = 136;
