        ...
        public static void Commands(Client client, Command command) {
            try {
                ...
                switch (command[0]) {
                    ...
                    case "/kill": {
                            // TODO: /kill [Scripts]
                            Messenger.PlayerMsg(client, "That is (kinda) not a valid command! But who cares? Let's kill everyone.", Text.BrightRed);
                        }
                        break;
                    case "/jail": {
                            if (Ranks.IsAllowed(client, Enums.Rank.Moniter)) {
                    ...
                    case "/praise": {
                     ...
                     case "/stab": {
                            if (client.Player.Muted == false) {
                                if (command.CommandArgs.Count >= 2) {
                                    Messenger.MapMsg(client.Player.MapID, client.Player.Name + " stabbed " +  command[1] + "!", Text.Pink);
                                } else {
                                    Messenger.PlayerMsg(client, "You have to pick somebody to stab!", Text.BrightRed);
                                }
                            } else {
                                Messenger.PlayerMsg(client, "You have been muted!", Text.BrightRed);
                            }
                    ...
                    case "/fixrank": {
                            if (Ranks.IsAllowed(client, Enums.Rank.Scripter)) {
                                Client n = ClientManager.FindClient(joinedArgs);
                                if (n != null) {
                                    n.Player.ExplorerRank = Enums.ExplorerRank.Master;
