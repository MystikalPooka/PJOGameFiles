    ...
    public partial class Main {
        ...
        public static void JoinGame(Client client) {
            try {
                ...
                if (Ranks.IsDisallowed(client, Enums.Rank.Moniter)) {
                    ...
                    Messenger.MapMsg(client.Player.MapID, client.Player.Name + " has joined Pokemon Journey Online!", Text.BrightGreen);
                    Messenger.AdminMsg(client.Player.Name + " has joined Pokemon Journey Online!", Text.BrightGreen);
                    foreach (Client i in ClientManager.GetClients()) {
                        if (i.Player.IsFriend(client.Player.Name) && i.Player.MapID != client.Player.MapID) {
                            Messenger.PlayerMsg(i, client.Player.Name + " has joined Pokemon Journey Online!", Text.BrightGreen);
                        }
                    }
                } else {
                    if (Ranks.IsDisallowed(client, Enums.Rank.Admin)) {// && NetScript.GetPlayerName(client) != "Shockie") {
                        Messenger.GlobalMsg("[" + GetRankName(client.Player.Access) + "] " + client.Player.Name + " has joined Pokemon Journey Online!", Text.BrightGreen);
                    } else {
                    	Messenger.AdminMsg("[" + GetRankName(client.Player.Access) + "] " + client.Player.Name + " has joined Pokemon Journey Online!", Text.BrightGreen);
                    }
                ...
                Messenger.PlayerMsg(client, "Welcome to Pokémon Journey Online!", Text.White);
        ...
        public static void LeftGame(Client client) {
            try {
                ...
                if (Ranks.IsDisallowed(client, Enums.Rank.Moniter)) {
                    Messenger.MapMsg(client.Player.MapID, client.Player.Name + " has left Pokémon Journey Online!", Text.BrightGreen);
                    Messenger.AdminMsg(client.Player.Name + " has left Pokemon Journey Online!", Text.BrightGreen);
                    foreach (Client i in ClientManager.GetClients()) {
                        if (i.Player.IsFriend(client.Player.Name) && i.Player.MapID != client.Player.MapID) {
                            Messenger.PlayerMsg(i, client.Player.Name + " has left Pokémon Journey Online!", Text.BrightGreen);
                        }
                    }
                } else {
                    if (Ranks.IsDisallowed(client, Enums.Rank.Admin)) { //NetScript.GetPlayerName(index) != "Pikachu") {// && NetScript.GetPlayerName(index) != "Shockie") {
                        Messenger.GlobalMsg("[" + GetRankName(client.Player.Access) + "] " + client.Player.Name + " has left Pokémon Journey Online!", Text.BrightGreen);
                    } else {
                    	Messenger.AdminMsg("[" + GetRankName(client.Player.Access) + "] " + client.Player.Name + " has left Pokémon Journey Online!", Text.BrightGreen);
                    }
                }
				...
				*/
                ...
                string[] triggerWords = new string[] { 
                	...
                	"gook", "retard", "ass" , "arse", "assbag", "assbandit", "assbanger",
                	 ...
                	 "wank", "wetback", "whore", "whorebag", "wop", "paynis","arsehole", "stfu"
                };
            ...
            switch (npcNum) {
                ...
                case 31: {//chansey
                        ...
                        if (((Recruit)attacker).Owner.Player.HasItem(110) > 0 || ((Recruit)attacker).Owner.Player.HasItem(109) > 0 || ((Recruit)attacker).Owner.Player.HasItem(131) > 0) {
                            Messenger.AskQuestion(((Recruit)attacker).Owner, "TradeEgg", "Will you give us the egg at the bottom of your inventory?", Pokedex.FindByName("Chansey").ID);
                        } else {
                            Story story = new Story();
                            StoryBuilderSegment segment = StoryBuilder.BuildStory();
                            StoryBuilder.AppendSaySegment(segment, "If you've found an egg and don't know what to do with it, we'll be happy to take it in!  We'll give you a gift for it too!", Pokedex.FindByName("Chansey").ID, 0, 0);
                            segment.AppendToStory(story);
                            StoryManager.PlayStory(((Recruit)attacker).Owner, story);
                        }
                    }
                ...
                case 53: {//luvdisc
                        ...
                        if (((Recruit)attacker).Owner.Player.HasItem(702) > 0) {
                            Messenger.AskQuestion(((Recruit)attacker).Owner, "RecallEggMove", "Oh my!  A Heart Scale!  Will you let me teach you a random move for it?", Pokedex.FindByName("Luvdisc").ID);
                        } else {
                            Story story = new Story();
                            StoryBuilderSegment segment = StoryBuilder.BuildStory();
                            StoryBuilder.AppendSaySegment(segment, "Hi!  I'm a move tutor.  If you can find me a Heart Scale, I'll teach someone a move.", Pokedex.FindByName("Luvdisc").ID, 0, 0);
                            StoryBuilder.AppendSaySegment(segment, "But be warned, I don't know what move you'll get until after I teach it!", Pokedex.FindByName("Luvdisc").ID, 0, 0);
                            segment.AppendToStory(story);
                            StoryManager.PlayStory(((Recruit)attacker).Owner, story);
                        }
                    }
                ...
                case 350: {//sableye
                        ...
                        if (choices.Count > 0) {
				            ...
				            Messenger.AskQuestion(((Recruit)attacker).Owner, "LearnTutorMove", "Your shards!  They speak to me!  Give me one, and shall bestow upon you techniques unknown!", Pokedex.FindByName("Sableye").ID, choices.ToArray());
                        } else {
                            Story story = new Story();
                            StoryBuilderSegment segment = StoryBuilder.BuildStory();
                            StoryBuilder.AppendSaySegment(segment, "Tell me, do you have any shards?  Their history intrigues me...", Pokedex.FindByName("Sableye").ID, 0, 0);
                            StoryBuilder.AppendSaySegment(segment, "If you can find me any ancient shards, I can teach you a special move.", Pokedex.FindByName("Sableye").ID, 0, 0);
                            segment.AppendToStory(story);
                            StoryManager.PlayStory(((Recruit)attacker).Owner, story);
                        }
                    }
        ...
        public static void StoryScript(Client client, int scriptNum, string param1, string param2, string param3, bool paused) {
            ...
            switch (scriptNum) {
	            ...
	            case 71: {//teach tutor move
	            	...
	            	break;
                case 72: {
                        BossBattles.RunPostIntroStoryBossBattle(client, "BoltStrikeCrater");
                    }
                    break;
            }
            } catch (Exception ex) {
        ...
        public static void ThrewItem(BattleSetup setup, InventoryItem item, int invNum) {
            try {
                if (setup.Attacker.CharacterType == Enums.CharacterType.Recruit) {
                    setup.PacketStack.AddPacket(((Recruit)setup.Attacker).Owner, PacketBuilder.CreateChatMsg("Throwing items currently in progress.", Text.Magenta));
                }
            } catch (Exception ex) {
