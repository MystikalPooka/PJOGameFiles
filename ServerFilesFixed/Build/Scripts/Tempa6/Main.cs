    ...
    public partial class Main {
        ...
        public static void JoinGame(Client client) {
            try {
                ...
                if (Ranks.IsDisallowed(client, Enums.Rank.Moniter)) {
                    ...
                    Messenger.MapMsg(client.Player.MapID, client.Player.Name + " has joined Pokémon Journey Online!", Text.Yellow);
                    Messenger.AdminMsg(client.Player.Name + " has joined Pokémon Journey Online!", Text.Yellow);
                    foreach (Client i in ClientManager.GetClients()) {
                        if (i.Player.IsFriend(client.Player.Name) && i.Player.MapID != client.Player.MapID) {
                            Messenger.PlayerMsg(i, client.Player.Name + " has joined Pokémon Journey Online!", Text.Yellow);
                        }
                    }
                } else {
                    if (Ranks.IsDisallowed(client, Enums.Rank.Admin)) {// && NetScript.GetPlayerName(client) != "Shockie") {
                        Messenger.GlobalMsg("[" + GetRankName(client.Player.Access) + "] " + client.Player.Name + " has joined Pokémon Journey Online!", Text.Yellow);
                    } else {
                    	Messenger.AdminMsg("[" + GetRankName(client.Player.Access) + "] " + client.Player.Name + " has joined Pokémon Journey Online!", Text.Yellow);
                    }
        ...
        public static void LeftGame(Client client) {
            try {
                ...
                if (Ranks.IsDisallowed(client, Enums.Rank.Moniter)) {
                    Messenger.MapMsg(client.Player.MapID, client.Player.Name + " has left Pokemon Journey Online!", Text.Yellow);
                    Messenger.AdminMsg(client.Player.Name + " has left Pokemon Journey Online!", Text.Yellow);
                    foreach (Client i in ClientManager.GetClients()) {
                        if (i.Player.IsFriend(client.Player.Name) && i.Player.MapID != client.Player.MapID) {
                            Messenger.PlayerMsg(i, client.Player.Name + " has left Pokemon Journey Online!", Text.Yellow);
                        }
                    }
                } else {
                    if (Ranks.IsDisallowed(client, Enums.Rank.Admin)) { //NetScript.GetPlayerName(index) != "Pikachu") {// && NetScript.GetPlayerName(index) != "Shockie") {
                        Messenger.GlobalMsg("[" + GetRankName(client.Player.Access) + "] " + client.Player.Name + " has left Pokemon Journey Online!", Text.Yellow);
                    } else {
                    	Messenger.AdminMsg("[" + GetRankName(client.Player.Access) + "] " + client.Player.Name + " has left Pokemon Journey Online!", Text.Yellow);
                    }
                }
            ...
            try {
                switch (scriptNum) {
                    ...
                    case 28: {//tanren chambers
                        ...
                        break;
                    case 29: {//completed Frozen Ruins
                            client.Player.IncrementDungeonCompletionCount(43, 1);
                            //Messenger.PlayerWarp(client, 1015, 25, 25);
                            DungeonRules.ExitDungeon(client, 1015, 25, 25);
                        }
                        break;
                    case 30: {//completed Underground Ruins
                            client.Player.IncrementDungeonCompletionCount(42, 1);
                            //Messenger.PlayerWarp(client, 1015, 25, 25);
                            DungeonRules.ExitDungeon(client, 1015, 25, 25);
                        }
                        break;
                ...
                switch (questionID) {
                    ...
                    case "Dig": {
                            if (answer == "Yes") {
                                int surfSlot = -1;
                                for (int i = 0; i < Constants.MAX_PLAYER_MOVES; i++) {
                                    if (client.Player.GetActiveRecruit().Moves[i].MoveNum == 146) {
                                        surfSlot = i;
                                    }
                                }
                    ...
                    case "CliffsideTablet": {
                            if (answer == "Yes") {
                                if (client.Player.FindInvSlot(-1) > 0) {
                                    ...
                                    Messenger.PlayerMsg(client, "You obtained a Shining Star!", Text.Yellow);
                                } else {
