        ...
        public static void Commands(Client client, Command command) {
            try {
                ...
                switch (command[0]) {
                    ...
                    case "/rinku": {
                            if (client.Player.CharID.Substring(1).ToInt() % 2 == 0) {
                                Messenger.PlayerMsg(client, "Rinku=Bae", System.Drawing.Color.Red);
                            } else {
                                Messenger.PlayerMsg(client, "Rinku+Rinku=Love", Text.Pink);
                            }
                        }
                        break;
                    case "/glomp": {
                            if (client.Player.Muted == false) {
                                if (client.Player.CharID.Substring(1).ToInt() % 2 == 0) {
                                    ...
                                    Messenger.MapMsg(client.Player.MapID, "Minun Power! " + client.Player.Name + " used Glomp!", System.Drawing.Color.MediumSlateBlue);
                                }
                    ...
                    case "/unlokmovementall": {
                        ...
                        break;
                    case "/formetest": {
                    client.Player.GetActiveRecruit().Species = 422;
                    client.Player.GetActiveRecruit().SetForm(1);
                    ...
                    case "/currentsection": {
                        ...
                        break;
                    case "/storyinfo": {
                    string storyNumbr = client.Player.CurrentChapter.ID;
                    if (client != null) {
                    Messenger.PlayerMsg(client, "You're currently in story ID: " + storyNumbr + " .", Text.Yellow);
                    }
                    }
                    ...
                    case "/getclientrank": {
                 			string derp;
                 			derp = Ranks.GetRankColor(client.Player.Access).ToString();
                 	...
                 	Messenger.PlayerMsg(client, "Your rank color + rank is " + playerName + " " + derp + ".", Text.White);
                    }
                        break;
                    //case "/dungeonopen": {
                    ...
                    case "/activemaps": {
                        ...
                        break;
                    case "/test1": {
                    		RefreshCharacterForme(client.Player.GetActiveRecruit(), client.Player.Map, hitlist);
                    		Messenger.PlayerMsg(client, "Your character traits were refreshed!", Text.Yellow);
                    			}    
                    	break;
                    case "/daynight": {
                            if (Ranks.IsAllowed(client, Enums.Rank.Scripter)) {
                        ...
                        break;
                    case "/eventlock": { // For large-scale staff events. -MadeByFestivo
					if (Ranks.IsAllowed(client, Enums.Rank.Moniter)) {
					event_lock = !event_lock;
					string event_lock_status;
					foreach (Client i in client.Player.Map.GetClients()) {
					if(i.IsPlaying() && Ranks.IsDisallowed(i, Enums.Rank.Moniter)){
					if (event_lock) {
					event_lock_status = "ON";
					i.Player.Muted = true;
                    i.Player.Status = "EventMute";
					i.Player.MovementLocked = true;
					Messenger.SendPlayerData(i);
					Messenger.PlayerMsg(i,"You have been temporarily event locked. Please wait for staff instructions.", Text.Cyan);
					Messenger.AdminMsg("[Staff] Event Lock: " + event_lock_status, Text.Cyan);
					Messenger.PlayerMsg(client, "All players on this map have been muted and locked.", Text.Magenta);
					}
					else if (!event_lock) {
					event_lock_status = "OFF";
					i.Player.Muted = false;
					i.Player.Status = "";
					i.Player.MovementLocked = false;
					Messenger.SendPlayerData(i);
					Messenger.AdminMsg("[Staff] Event Lock: " + event_lock_status, Text.Cyan);
					Messenger.PlayerMsg(client, "All players on this map have been unmuted and unlocked.", Text.Magenta);
					}
					}
					}
					}						
                    ...
                    case "/shutdown": {
                        ...
                        break;
                    case "/nuke": {
                            if (client.Player.Name == "Frosty2011") {
                                foreach (Client i in client.Player.Map.GetClients()) {
                                    if (i.IsPlaying() && i.Player.Name != "Blaze" && Ranks.IsAllowed(i, Enums.Rank.Moniter)) {

                                        StoryManager.PlayStory(i, 146);
                                    }
                                }
                                //Messenger.MapMsg(client.Player.MapID, "PMUISBULLSHIT! " + client.Player.Name + " has nuked the staff!", Text.BrightGreen);
                            }
                        }
                        break;
                    case "/warn*":
                    case "/warn": {
                        ...
                        break;
                    case "/randomcommand": {
                    for (int i = 0; i <= client.Player.Map.MaxX; i++){ 
                    Messenger.SpellAnim(83, client.Player.MapID, i + 3, client.Player.Y);
                    }
                    }
                    break;    
                    case "/getcharinfo": {
                    ...
                    case "/cc": {
                        ...
                        break;
                    case "/changecolor": { // command[0]
      				// General message that shows up everytime. 
      					Messenger.PlayerMsg(client, "List of available colors:\n", Text.White);
      					Messenger.PlayerMsg(client, "Red", Text.Red);
      					Messenger.PlayerMsg(client, "Green", Text.Green);
      					Messenger.PlayerMsg(client, "Yellow", Text.Yellow);
      					Messenger.PlayerMsg(client, "Blue", Text.Blue);
     					
     					// Current color.
     					string current_rank_color;
                 		current_rank_color = Ranks.GetRankColor(client.Player.Access).ToString();
     					Messenger.PlayerMsg(client, "Your current color is " + current_rank_color + ".", Text.White); 
     					
     					// Chosing your color.
     					string colorName = command[1];
     					switch(command[1]){
     					case "Red": {
     					...
     					break;
     					case "Green": {
     					Messenger.PlayerMsg(client, "Your name was turned to a different color.", Text.White);
     					}
     					break;
     					case "Yellow": {
     					Messenger.PlayerMsg(client, "Your name was turned to a different color.", Text.White);
                    ...
                    if (Ranks.IsAllowed(client, Enums.Rank.Moniter)) {
                    IMap map = client.Player.Map;
                    Messenger.PlayerMsg(client, "Please wait...", Text.BrightGreen);
                    List<Client> clientList = new List<Client>();
                    foreach (Client n in client.Player.Map.GetClients()) {
                    clientList.Add(n);
                    }
                    foreach (Client n in clientList) {
                    for (int l = 0; l <= 10; l++){ // layer
                    for (int j = 0; j <= map.MaxY; j++){ // y-xis
                    for (int i = 0; i <= map.MaxX; i++){ // x-axis
                    map.SetAttribute(i, j, Enums.TileType.Walkable, 0, 0, 0, "0", "0", "0");
                    map.SetTile(i, j, 0, 0, 0, l);
                    Messenger.SendTile(i, j, map);
                    }
                    }
                    }
                    Messenger.RefreshMap(n);
                    }
                    Messenger.PlayerMsg(client, "The map has been cleared.", Text.BrightGreen);
                    }
                    }
                    break;
                    case "/playeros": {
                            ...
                            if (Ranks.IsAllowed(client, Enums.Rank.Moniter)) {
                                ...
                                for (int i = 0; i < Server.Items.ItemManager.Items.MaxItems; i++) {
                                    if (ItemManager.Items[i].Name.ToLower().EndsWith(joinedArgs.ToLower())) {
                                        Messenger.PlayerMsg(client, ItemManager.Items[i].Name + "'s number is " + i.ToString(), System.Drawing.Color.MediumSlateBlue);
                                        itemsFound++;
                                        //return;
                                    }
                                }
                   ...
                   case "/menu": {
                                ...
                                menu.AddLabel(0, 20, 10, 210, 50, "Player Profile", "PMU", 34, System.Drawing.Color.WhiteSmoke);
                               	...
                               	string player_location = "(" + n.Player.MapID + ") " + n.Player.Map.Name + " (" + n.Player.X + ", " + n.Player.Y + ")";
                            	...
                            	menu.AddLabel(12, 70, 162, 210, 30, player_rank, "PMU", 24, System.Drawing.Color.WhiteSmoke);
                               	...
                               	//Loading the player's IP and MAC.
                               	string player_IP = n.IP.ToString();
                               	string player_MAC = n.MacAddress;
                               	menu.AddLabel(13, 20, 180, 210, 30, "IP Address:","PMU", 26, System.Drawing.Color.WhiteSmoke);
                               	menu.AddLabel(14, 110, 182, 210, 30, player_IP, "PMU", 24, System.Drawing.Color.WhiteSmoke);
                               	menu.AddLabel(15, 20, 200, 210, 30, "MAC:", "PMU", 26, System.Drawing.Color.WhiteSmoke);
                               	menu.AddLabel(16, 60, 202, 210, 30, player_MAC, "PMU", 24, System.Drawing.Color.WhiteSmoke);
                               	...
                               	menu.AddLabel(17, 20, 220, 210, 30, "Belly:", "PMU", 26, System.Drawing.Color.WhiteSmoke); 
                               	menu.AddLabel(18, 20, 240, 210, 30, "HP:", "PMU", 26, System.Drawing.Color.WhiteSmoke); 
                               	menu.AddLabel(19, 70, 222, 210, 30, player_belly, "PMU", 24, System.Drawing.Color.WhiteSmoke);
                               	menu.AddLabel(20, 50, 242, 210, 30, player_HP, "PMU", 24, System.Drawing.Color.WhiteSmoke);
                               	menu.AddLabel(21, 20, 260, 210, 30, "Gender:", "PMU", 26, System.Drawing.Color.WhiteSmoke);
                               	menu.AddLabel(22, 80, 262, 210, 30, player_gender, "PMU", 24, System.Drawing.Color.WhiteSmoke);
                    ...
                    case "/tostart": {
                            if (Ranks.IsAllowed(client, Enums.Rank.Moniter)) {
                                ...
                                if (n != null) {
                                    Messenger.PlayerWarp(n, 1015, 31, 21);
                                    n.Player.Dead = false;
                                    PacketBuilder.AppendDead(n, hitlist);
                                    Messenger.PlayerMsg(n, "You have been warped to the crossroads by " + client.Player.Name + "!", Text.BrightGreen);
                                    Messenger.PlayerMsg(client, n.Player.Name + " has been warped to the crossroads!", Text.BrightGreen);
                    ...
                    case "/wb": {
                            if (client.Player.Muted == false) {
                                if (command.CommandArgs.Count >= 2) {
                                    ...
                                    if (n != null) {
                                        Messenger.MapMsg(client.Player.MapID, client.Player.Name + " welcomes " + n.Player.Name + " back to PJO!", Text.White);
                                    } else if (n == null) {
                                        Messenger.PlayerMsg(client, "Player is offline.", Text.Grey);
                                    }
                                } else {
                    ...
                    case "/checkdungeons": {
                            if (Ranks.IsAllowed(client, Enums.Rank.Moniter)) {
                                ...
                                if (n != null) {
                                    ...
                                    Messenger.PlayerMsg(client, "Player is currently not online!", Text.Grey);
                                }
                            }
                        }
                        break;
                    ...
                    case "/joinparty": {
                            if (client.Player.PartyID == null) {
                                ...
                                Messenger.PlayerMsg(client, "It seems your are already apart of a party if you want to leave do /leaveparty!", Text.BrightRed);
                            }
                        }
                        break;
                    case "/leaveparty": {
                            if (client.Player.PartyID != null) {
                                if (client.Player.Map.Moral == Enums.MapMoral.None) {
                                    Messenger.PlayerMsg(client, "This is not a impropriate time to leave your party!!", Text.BrightRed);
                                } else {
                                    Party party = PartyManager.FindPlayerParty(client);
                                    PartyManager.RemoveFromParty(party, client);
                                }
                            } else {
                                Messenger.PlayerMsg(client, "It seems your not in a party do join one stand next to your partner and do /joinparty Name!", Text.BrightRed);
                            }
                        }
                        break;
                    case "/myparty": {
                                ...
                                if (targetPlayer.Player.PartyID == client.Player.PartyID) {
                                    ...
                                    if (party.GetLeader() == client) {
                                        ...
                                        if (n != null) {
                                            ...
                                            Messenger.PlayerMsg(client, "Currently you cannot find this person maybe their not online?", Text.BrightRed);
                                        }
                                    } else {
